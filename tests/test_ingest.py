...
def test_distant_cues_stay_separate_and_sorted():
    ...
    cue_windows = [window for window in windows if not window.tail]
    assert [w.t_start for w in cue_windows] == sorted(w.t_start for w in cue_windows)
    assert len(cue_windows) == 2
    assert [w.id for w in cue_windows] == ["w-001", "w-002"]
...
def test_window_bounds_clamped_to_video():
    ...
    cue_windows = [window for window in windows if not window.tail]
    assert cue_windows[0].t_start == 0.0
    assert cue_windows[-1].t_end == 100.0
...
def test_cap_keeps_strongest_cues():
    ...
    cue_windows = [window for window in windows if not window.tail]
    assert len(cue_windows) == 40
    # The weakest cues (earliest times here) must be the ones dropped.
    kept_scores = sorted(w.score for w in cue_windows)
